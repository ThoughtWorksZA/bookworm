    ...
    public class HomeControllerTest
    {
        ...
        public void IndexShouldHave5LatestPostsOrBookPosts()
        {
            using (var session = _documentStore.OpenSession())
            {
                ...
                repository.Create(new Book { Posts = new List<BookPost> { new BookPost { Id = 2, Title = "BookPost", Content = "Derp", UpdatedAt = DateTime.Now.AddMinutes(1) } } });
                repository.Create(new Post { Id = 3, Title = "Post", Content = "Derp", CreatedAt = DateTime.Now.AddMinutes(2) });
                repository.Create(new Book { Posts = new List<BookPost> { new BookPost { Id = 4, Title = "BookPost", Content = "Derp", UpdatedAt = DateTime.Now.AddMinutes(2) } } });
                repository.Create(new Post { Id = 5, Title = "Post", Content = "Derp", CreatedAt = DateTime.Now.AddMinutes(3) });
                repository.Create(new Book { Posts = new List<BookPost> { new BookPost { Id = 6, Title = "BookPost", Content = "Derp", UpdatedAt = DateTime.Now.AddMinutes(3) } } });
                var post = new Post { Id = 101, Title = "Latest Post", Content = "Derp", CreatedAt = DateTime.Now.AddMinutes(10) };
                repository.Create(post);
                var bookPost = new Book { Posts = new List<BookPost> { new BookPost { Id = 102, Title = "Latest BookPost", Content = "Derp", UpdatedAt = DateTime.Now.AddMinutes(10) } } };
                repository.Create(bookPost);
                repository.Create(new Post { Id = 7, Title = "Post", Content = "Derp", CreatedAt = DateTime.Now.AddMinutes(4) });
                repository.Create(new Book { Posts = new List<BookPost> { new BookPost { Id = 8, Title = "BookPost", Content = "Derp", UpdatedAt = DateTime.Now.AddMinutes(4) } } });
                repository.Create(new Post { Id = 9, Title = "Post", Content = "Derp", CreatedAt = DateTime.Now.AddMinutes(5) });
                repository.Create(new Book { Posts = new List<BookPost> { new BookPost { Id = 10, Title = "BookPost", Content = "Derp", UpdatedAt = DateTime.Now.AddMinutes(5) } } });
                session.SaveChanges();
